...
module.exports = {
    ...
    orderSummery:()=>{
        return new Promise((resolve,reject)=>{
        db.get().collection(collection.ORDER_COLLECTION).aggregate([{$match:{}},{$sort:{_id:-1}},{$limit:1},
            {
                $unwind: '$products'
            },
             {
...
findOrderSummery:(oId)=>{
    console.log('reached method')
    console.log(oId);
    return new Promise((resolve,reject)=>{
        db.get().collection(collection.ORDER_COLLECTION).findOne({_id:oId}).then((response)=>{
            console.log(response);
            resolve(response)
        })
    })
